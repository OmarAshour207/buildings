...
<body style="direction: rtl">
    ...
    <div class="container"> <a class="navbar-brand pull-right" href="{{ url('/') }}"><i class="fa fa-paper-plane"></i> ONE</a>
        <div class="menu pull-left"> <a class="toggleMenu" href="#"><img src="{{ asset('website/images/nav_icon.png') }}" alt="" /> </a>
            <ul class="nav" id="nav">
                <li class="{{ setActive([''], 'current') }}"><a href="{{ url('/') }}"> الصفحه الرئيسيه <i class="fa fa-home"></i></a></li>
                <li  class="{{ setActive(['all_buildings'], 'current') }}"><a href="{{ url('/all_buildings') }}"> كل العقارات <i class="fa fa-building-o"></i></a></li>
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right {{ setActive(['search'], 'current') }}" aria-labelledby="navbarDropdown">
                        @foreach(building_type() as $keyType => $type)
                            <li style="display: block"> <a class="dropdown-item" href="{{ url('search?rent=1&type='.($keyType+1)) }}"> {{ $type }} </a> </li>
                        @endforeach
                    </ul>
                </li>
                ...
                <li class="nav-item dropdown {{ setActive(['search'], 'current') }}">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
                        @foreach(building_type() as $keyType => $type)
                            <li style="display: block"> <a class="dropdown-item" href="{{ url('search?rent=2&type='.($keyType+1)) }}"> {{ $type }} </a> </li>
                        @endforeach
                    </ul>
                ...
                <li  class="{{ setActive(['contact']) }}">
                    <a href="{{ url('/contact') }}"> أتصل بنا <i class="fa fa-phone"></i></a>
                </li>
                @guest
                    <li class="nav-item  {{ setActive(['login']) }}">
                        <a class="nav-link" href="{{ route('login') }}"> تسجيل الدخول <i class="fa fa-arrow-right"></i></a>
                    </li>
                    @if (Route::has('register'))
                        <li class="nav-item {{ setActive(['login']) }}">
                            <a class="nav-link" href="{{ route('register') }}"> تسجيل عضويه <i class="fa fa-home"></i></a>
                        </li>
                    @endif
                @else
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
                            ...
                            <hr>
                            <a class="dropdown-item"  href="{{ url('user/unproved/stores') }}">
                                <i class="fa fa-building-o"></i>
                                عقاراتي الغير مفعله
                            </a>
                            <hr>
    ...
    <div class="footer">
        <div class="footer_bottom">
            <div class="follow-us">
                <a class="fa fa-facebook social-icon" href="{{ setting()->facebook }}"></a>
                <a class="fa fa-twitter social-icon" href="{{ setting()->facebook }}"></a>
                <a class="fa fa-instagram social-icon" href="{{ setting()->instagram }}"></a>
            </div>
            <div class="copy">
